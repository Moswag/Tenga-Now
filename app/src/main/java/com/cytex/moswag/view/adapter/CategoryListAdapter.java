...
public class CategoryListAdapter extends
    ...
    public CategoryListAdapter(Context context,List<ProductCategoryModel> companies) {

        categoryList = companies;   //CenterRepository.getCenterRepository().getListOfCategory();
    ...
    public void onBindViewHolder(VersionViewHolder versionViewHolder,
        ...
        drawable = mDrawableBuilder.build(String.valueOf(categoryList
                        .get(categoryIndex).getCompanyName().charAt(0)),
                mColorGenerator.getColor(categoryList.get(categoryIndex)
                        .getCompanyName()));

        ImageUrl = categoryList.get(categoryIndex).getCompanyImageUrl();
